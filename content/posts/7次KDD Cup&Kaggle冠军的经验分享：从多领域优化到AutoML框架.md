...
![图1 国际顶级比赛冠军经历](https://p1.meituan.net/travelcube/eb196311c6a5f6c30736503879113e99113330.png)
...
![图2 KDD Cup近20年问题趋势](https://p0.meituan.net/travelcube/4c79b792501827e6dd69f82453f7bc8292349.png)
...
![表1 竞赛及解决方案](https://p1.meituan.net/travelcube/fb07e988cb4d824605f31d08f00980c7189040.png)
...
**竞赛问题与挑战**：竞赛要求在Outbrain网页内容发现平台上，预估用户下一次点击网页广告，具体参考：[Kaggle Outbrain比赛介绍详�](https://www.kaggle.com/c/outbrain-click-prediction)�\[26\]。参赛选手会面对以下两个重要挑战：
...
![图3 多层级多因子模型融合](https://p0.meituan.net/travelcube/d3d8ac1b80f4b12e96934d59d6c08543213246.png)
...
**竞赛问题与挑战**：竞赛是以电子商务平台为背景，预估用户下一次点击的商品。并围绕着如何缓解推荐系统中的选择性偏差以及流行度偏差进行展开，具体参考：[KDD Cup 2020 Debiasing比赛介绍详�](https://tianchi.aliyun.com/competition/entrance/231785/introduction)�\[27\]。推荐系统中的偏差问题有很多，除了上述两种偏差，还有曝光偏差、位次偏差等等\[5\]\[6\]。我们团队之前也对位次偏差进行了相关研究\[7\]。而本次竞赛为了更好地衡量推荐系统对历史低热度商品的推荐效果，选手的成绩主要采用NDCG@50\_half指标进行排名。该指标是从整个评测数据集中取出一半历史曝光少的点击商品，由于是低热度且有被点击的商品，可以跟更好的评估偏差问题。本次比赛包含了以下挑战：



* 赛题只提供点击数据，构造候选集时需要考虑选择性偏差问题。
* 不同商品热度差异大，商品历史点击次数呈现一个长尾分布，数据存在严重的流行度偏差问题，并且评估指标 NDCG@50\_half 用于考察低热度商品的排序质量。
...
![图4 基于i2i的建模框架](https://p0.meituan.net/travelcube/6e488872f26502ed198e275a20f1cabc195934.png)
...
![](https://p0.meituan.net/travelcube/3eb5103bedf49baa4d089f107d899ced31225.png)
...
关于该比赛的更多细节，大家可以参考《[KDD Cup 2020 Debiasing比赛冠军技术方案及在美团的实践](https://tech.meituan.com/2020/08/20/kdd-cup-debiasing-practice.html)》一文。
...
**竞赛问题及挑战**：竞赛目标是预测北京和伦敦48个站点在未来48小时里PM2.5/PM10/O3的浓度变化，具体参考： [KDD Cup 2018比赛介绍详�](https://www.biendata.xyz/competition/kdd_2018/)�\[28\]。参赛选手需要解决以下两个挑战：
...
![图5 时空挑战图](https://p0.meituan.net/travelcube/d7a0144f3a2eb366e61d92867a62f4d7284361.png)
...
![图6 Spatial-temporal Gated DNN](https://p1.meituan.net/travelcube/c4ba1908f5389b3ddcde6b8f8e7a4f2b125205.png)
...
![图7 Seq2Seq模型](https://p0.meituan.net/travelcube/97f50c11ae4fde0237cd8dee960c7f47433435.png)
...
更多详情，大家可参考SIGKDD会议论文：[AccuAir: Winning Solution to Air Quality Prediction for KDD Cup 2018](https://dl.acm.org/doi/10.1145/3292500.3330787)。
...
**竞赛问题及挑战**：竞赛目标是以20分钟为时间窗口，给定前2小时高速公路入口到关卡的行驶状况，预测未来2小时的行驶状况，具体可参考：[KDD Cup 2017比赛介绍详�](https://tianchi.aliyun.com/competition/entrance/231597/introduction)�\[29\]。竞赛根据行驶状况的不同，分为了行驶时间预测和交通流量预测两个赛道。参赛选手需要解决以下两个挑战：
...
![图8 交通流量数据中的噪音](https://p0.meituan.net/travelcube/536766354a90d16de4c53233197aa243327182.png)
...
![](https://p0.meituan.net/travelcube/321a7738ff6b52a4d4f98b483e0c9e233326.png)
...
**竞赛问题及挑战**：自动化图表示学习挑战赛（AutoGraph）是第一个应用于图结构数据的AutoML挑战，详情请见[KDD Cup 2020 AutoGraph 比赛介绍](https://www.automl.ai/competitions/3)\[30\]。竞赛选择图结点多分类任务来评估表示学习的质量，参与者需设计自动化图表示学习\[11\-13\]解决方案。该方案需要基于图的给定特征、邻域和结构信息，高效地学习每个结点的高质量表示。比赛数据从真实业务中收集，包含社交网络、论文网络、知识图谱等多种领域共15个，其中5个数据集可供下载，5个反馈数据集评估方案在公共排行榜的得分，剩余5个数据集在最后一次提交中评估最终排名。



![图9 AutoGraph图数据集概览](https://p0.meituan.net/travelcube/c219df3dc35df51fce75f34bd679f652102119.png)
...
![图10 AutoHEnsGNN框架](https://p1.meituan.net/travelcube/e3e2127081a01e63fba8e810cde54f80201468.png)
...
**（2）层次模型集成**：这部分共包含两个维度的集成。第一层为**模型自集成**，为了解决图模型对初始化特别敏感，同种模型精度波动可达±1%的问题，采用了同模型的自集成，同时生成多个同种模型，并取模型预测的平均值作为该种模型的输出结果，成功降低了同种模型方差，提高了模型在不同数据集上的稳定性。第二层为**不同模型集成**，为了有效地利用来自本地和全球邻域的信息，充分捕获图的不同性质，我们采用加权集成了不同种类的图模型，进一步提高性能。同时针对在参数搜索阶段，需要同时优化模型内参数α，以及多种模型加权集成参数β，使用模型集成参数和模型内参数通过互迭代的梯度下降进行求解，有效提升了速度。



![图11 多类别层次化图模型优化](https://p0.meituan.net/travelcube/81871ecbed7b492564ef578f9d8eb99c324098.png)
...
具体详情，大家可参考团队ICDE 2022论文，[AutoHEnsGNN: Winning Solution to AutoGraph Challenge for KDD Cup 2020](https://arxiv.org/pdf/2111.12952.pdf)。
...
![图12 AutoML整体框架](https://p0.meituan.net/travelcube/ffa383ce902a4e6e75e07fb623720b8e305075.png)
...
![图13 自动化特征工程](https://p0.meituan.net/travelcube/4da27ffffaf35795da196014e608c520125654.png)
...
![图14 高阶算子特征产出方式](https://p0.meituan.net/travelcube/62195075168d22ab7a112c159dcf5778229909.png)
...
![图15 自动化模型优化](https://p1.meituan.net/travelcube/cb483aa0dbdeb14264e890ac42dbf12a118050.png)
...
![图16 多阶特征交叉](https://p1.meituan.net/travelcube/5c753c2c89c65e5fb4eec13c098528a8171327.png)
...
![图17 三阶段算法建模](https://p0.meituan.net/travelcube/b62a5af49ee13bdf30cb65da450e83f395744.png)
...
![图18 自动化建模框架](https://p0.meituan.net/travelcube/66cade648c66e88f79081b9ef1aed00088593.png)
...
相比较而言，比赛中的Spatial\-temporal Gated DNN更注重时空融合信息对于预测值的影响，由于需要预测的时间序列问题，更侧重于不同的时间、空间信息有能够将差异性建模充分。而在美团业务中的时空网络注重于细粒度刻画空间信息，源于不同的球面距离，不同的区块位置影响大，需要多重信息深度建模。更多详情，大家可参考团队的CIKM论文：[Trilateral Spatiotemporal Attention Network for User Behavior Modeling in Location\-based Search](https://dl.acm.org/doi/abs/10.1145/3459637.3482206)\[23\]。



![图19 基于三边时空注意力机制的用户行为序列网络](https://p0.meituan.net/travelcube/7873bb7ecea8fbf74cad1a3beae3385f317543.png)



在实际建模中，相对于比赛涉及到更多线上部分，而比赛主要专注于离线数据集的精度极值。同Debiasing比赛相比，在实际线上系统中，涉及到Bias等更多的问题，以Position Bias为例，实际的展示数据高位点击率天然高于低位，然而一部分是源于用户高低位之间的浏览习惯差异，因此对于数据的直接建模不足以表征对于高低位广告点击率与质量的评估。我们在美团实际广告系统中，设计了位置组合预估框架进行建模，取得不错的效果，这里不再详述。具体详情，大家可参考团队SIGIR论文：[Deep Position\-wise Interaction Network for CTR Prediction](https://arxiv.org/pdf/2106.05482.pdf)\[7\]。
...
* \[8\] [KDD Cup 2020 Debiasing比赛冠军技术方案及在美团的实践](https://tech.meituan.com/2020/08/20/kdd-cup-debiasing-practice.html).
...
* \[24\] [广告深度预估技术在美团到店场景下的突破与畅想](https://tech.meituan.com/2021/10/14/breakthrough-and-prospect-of-deep-ctr-prediction-in-meituan-ads.html).
* \[25\] Geurts P . Bias vs Variance Decomposition for Regression and Classification\[J\]. Springer US, 2005
* \[26\] Kaggle Outbrain比赛链接： [https://www.kaggle.com/c/outbrain\-click\-prediction](https://www.kaggle.com/c/outbrain-click-prediction).
* \[27\] KDD Cup 2020 Debiasing比赛链接 [https://tianchi.aliyun.com/competition/entrance/231785/introduction](https://tianchi.aliyun.com/competition/entrance/231785/introduction).
* \[28\] KDD Cup 2018比赛链接：[https://www.biendata.xyz/competition/kdd\_2018/](https://www.biendata.xyz/competition/kdd_2018/).
* \[29\] KDD Cup 2017比赛链接：[https://tianchi.aliyun.com/competition/entrance/231597/introduction](https://tianchi.aliyun.com/competition/entrance/231597/introduction).
* \[30\] KDD Cup 2020 AutoGraph比赛链接：[https://www.automl.ai/competitions/3](https://www.automl.ai/competitions/3)
