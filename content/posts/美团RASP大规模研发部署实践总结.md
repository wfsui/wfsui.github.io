...
date: 2024-02-27T02:39:56+0000
tags: [美团技术团队, 美团安全, 信息安全, RASP, Java, 安全防护]
---
...
![图1 公司JDK版本分布占比](https://p0.meituan.net/travelcube/146ad756e7f439ee154c6c37b436c200145359.png)
...
![图2 主机安全防护产品与业务的隔离等级](https://p0.meituan.net/travelcube/75a7c71de09f552374a2f4ce4328b910189182.png)
...
![图3 运行时注入cpu.busy指标抖动情况](https://p0.meituan.net/travelcube/9db83dae43d7a8c6e0a15d50f13610bb154102.png)
...
![图4 运行时注入TP9999指标抖动情况](https://p0.meituan.net/travelcube/b258c67260ef8c1bf242a9389baf6bb2121540.png)
...
![图5 运行时Java Agent的实现原理与升级过程](https://p0.meituan.net/travelcube/ef0070bd8a5d1a0698e2213632128e31348656.png)
...
![图6 RASP各类故障占比](https://p0.meituan.net/travelcube/7ff497a180def6baef5b61f7d1fb0992171713.png)
...
美团 RASP 利用 Java agent 和`instrumentation`技术，通过 ASM 修改类字节码，实时分析检测命令执行、文件访问、反序列化、JNDI、SQL注入等入侵行为。它最初是从开源项目[btrace](https://github.com/btraceio/btrace) 演化而来，后使用Golang重写了btrace的进程注入的功能，即架构中的 RASP Daemon 部分，在 Java Agent 端也参考了一些开源项目和公司内部的性能诊断工具。经过多年的迭代，RASP 逐渐形成目前的架构。
...
![图7 RASP的配置分发流程](https://p0.meituan.net/travelcube/7a0d0a32b27c2ff4fcae33c87bf5e791153367.png)
...
**Java Agent兼容性**：公司有多种Java Agent 包括性能诊断，安全扫描、动态调试、流量录制、热部署、链路追踪等约十多种，这些工具实现原理都是基于[Instrument](https://docs.oracle.com/en/java/javase/17/docs/api/java.instrument/java/lang/instrument/Instrumentation.html)。 冲突主要在还是在字节码修改上，例如RASP与[jdwp](jdwp)的兼容上，最初版本的RASP在业务类中增加方法数量，当用户开启远程debug时，本地代码的方法数量与远程不一样，导致JVM崩溃。Java Agent应该遵循的规范：
...
![图8 mt-rasp jar包拆分前后对比](https://p0.meituan.net/travelcube/7c3d16c5933494f10bf785aba30fe24b1369113.png)
...
![图9 RASP的类加载器层次结构](https://p0.meituan.net/travelcube/a482a765b23d7aa6bfb3ff66d75f2f76394001.png)
...
![图10 cpu.busy指标优化前后对比](https://p0.meituan.net/travelcube/ad32739bff391ef3bb82eb8f0ec55a97154252.png)
...
`premain`一期方案主要用来快速解决`cpu.busy`抖动问题，但是对于性能比较敏感（如tp9999 \< 50ms）的业务，运行时字节码修改不可避免的造成STW，从而导致TP9999 线升高。因为字节码修改时需要进入JVM的safepoint，执行字节码转换的方法[VM\_RedefineClasses::doit](https://github.com/openjdk/jdk/blob/jdk-11%2B28/src/hotspot/share/prims/jvmtiRedefineClasses.cpp) 会导致应用短暂地暂停响应，这部分代码执行慢就会影响应用`TP9999` 。大批量修改字节码测得火焰图如下图11所示：



![图11 批量字节码转换时的Java进程火焰图](https://p0.meituan.net/travelcube/d110ad8110c0d717b01463c5b402d0be971408.png)



从火焰图11看出，`RedefineClasses` 耗时主要在`VM_RedefineClasses::AdjustCpoolCacheAndVtable::do_klass` ，hotspot JDK官方也有类似的[issue](https://bugs.openjdk.org/browse/JDK-8046246) 。转换一个类的主要耗时在`redefine_single_class`方法，初步测试耗时占比在40%～80% 之间，并且一次转换类越多，占STW总耗时越大。
...
![图12 类转换STW时间与服务QPS、转换类数量的关系](https://p0.meituan.net/travelcube/1877431b1dab5e5989dde5ee05772c09122066.png)
...
![图13 RASP类加载器增加premain agent](https://p0.meituan.net/travelcube/75c9bd46575463d450f748d012f05838434041.png)
...
![图14 优化前后注入时TP9999指标](https://p1.meituan.net/travelcube/e631983f125316a63e2f5d9c7affa380125339.png)
...
![图15 RASP监控的指标分布](https://p1.meituan.net/travelcube/bbb1a9860d397fd6389c2595a7219054722757.png)
...
![图16 RASP阻断热修复控制流程](https://p0.meituan.net/travelcube/b3a68dbc9564a3a205a3db5eb88ab3f7403012.png)
